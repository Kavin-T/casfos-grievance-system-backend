...
const addComplaint = asyncHandler(async (req, res) => {
    const { raiserName, subject, date, details, department, premises, location, emergency } = req.body;
    ...
    if (req.files?.imgBefore) {
      ...
      const imgBeforeFinalPath = path.join(absoluteComplaintDir, imgBeforeFileName);

      // Move file
      fs.renameSync(imgBeforeTempPath, imgBeforeFinalPath);

      // Store relative path including 'uploads'
      imgBeforePath = path.join(complaintDir, imgBeforeFileName);
    }

    if (req.files?.vidBefore) {
      const vidBeforeTempPath = req.files.vidBefore[0].path;
      const vidBeforeFileName = `vidBefore_${savedComplaint._id}.mp4`;
      const vidBeforeFinalPath = path.join(absoluteComplaintDir, vidBeforeFileName);
